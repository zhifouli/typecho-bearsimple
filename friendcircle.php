...
<link href="<?php AssetsDir();?>assets/vendors/friendcircle/friendcircle.css?v=28" rel="stylesheet">
<link href="<?php AssetsDir();?>assets/vendors/friendcircle/plyr.min.css" rel="stylesheet">
<script src="<?php AssetsDir();?>assets/vendors/friendcircle/amrParse.js"></script>
...
<div class="pure-g" id="layout">
    <?php if(Bsoptions('fcircle_hiddenMode') == true):?>
    ...
    <div class="friendcircle-container">
        <section class="friendcircle-bg">
            <img src="<?php if(Bsoptions('fcircle_Background') == null): ?><?php AssetsDir();?>assets/images/mountain.webp<?php else:?><?php echo Bsoptions('fcircle_Background');?><?php endif;?>" class="friendcircle-bg-img">
...
<div id="getUpload">
				...
				<div class="ui icon tiny button" name="btn_v" id="btn_v"><i class="file video outline icon"></i> 上传音/视频</div>
				<ul id="ul_pics" class="ul_pics clearfix"></ul>
				<ul id="ul_video" class="ul_video clearfix"></ul>
		</div>
...
<script src="<?php AssetsDir();?>assets/vendors/friendcircle/moxie.js?v=2"></script>
<script src="<?php AssetsDir();?>assets/vendors/friendcircle/plupload.js"></script>

<script src="<?php AssetsDir();?>assets/vendors/friendcircle/plyr.min.js"></script>

<script src="<?php AssetsDir();?>assets/vendors/friendcircle/funlazy.min.js"></script>
...
FunLazy({
    placeholder: "<?php AssetsDir();?>assets/images/load.gif",
    effect: "fadeIn"
});
    function amrParse(){
    function E(selector) {
    ...
    $(".amr-begin").on('click', function () {
        let amr = new BenzAMRRecorder();
        let playBtn = E('#amr-play-'+$(this).attr('data-circleid'));
        let stopBtn = E('#amr-stop-'+$(this).attr('data-circleid'));
        let progressCtrl = E('#amr-progress-'+$(this).attr('data-circleid'));
        let isDragging = false;
        let cur = E('#amr-cur-'+$(this).attr('data-circleid'));
        let duration = E('#amr-duration-'+$(this).attr('data-circleid'));
        playBtn.prop('disabled', true);
        stopBtn.prop('disabled', true);
        progressCtrl.prop('disabled', true);
        amr.initWithUrl($(this).attr('data-amrfile')).then(function () {
            playBtn.prop('disabled', false);
            stopBtn.prop('disabled', false);
            progressCtrl.prop('disabled', false);
            progressCtrl.attr('max', amr.getDuration());
            duration.html(amr.getDuration().toFixed(2) + '\'');
        });
        $("#amr-begin-"+$(this).attr('data-circleid')).hide();
        $("#amr-play-"+$(this).attr('data-circleid')).fadeIn();
        $("#amr-stop-"+$(this).attr('data-circleid')).fadeIn();
         // 绑定事件
            amr.onPlay(function () {
                playBtn.html('<i class="pause icon">');
            });
            amr.onStop(function () {
                playBtn.html('<i class="play icon">');
            });
        ...
        if (amr) {
            cur.html(amr.getCurrentPosition().toFixed(2) + '\'');
            if (!isDragging) {
                progressCtrl.val(amr.getCurrentPosition().toFixed(2));
            }
        } else {
            cur.html('0\'');
        }
    ...
    progressCtrl.on('mousedown', function () {
        isDragging = true;
    });
    progressCtrl.on('mouseup', function () {
        isDragging = false;
    });
    progressCtrl.on('change', function (e) {
...
$("#commentform").append(tokenInput);
           ...
           $(".friendcircle-action").on('click mouseenter',function() {
            $(".tool-container").hide();
            $(this).find(".tool-container").show();
       
        });

      $(document).on('click',function(event) {
          if (!$(event.target).closest('.circle-action').length) {
              $(".tool-container").hide();
          }
         });
          $('.circle-pictures .circle-picture img,.friendcircle-avatar img,.friendcircle-bg img')
  .visibility({
    type       : 'image',
    transition : 'fade in',
    duration   : 1000
...
;
       function plyr(){
           if(!$(".circle_player").length){
            return false;   
           }
         const players = Plyr.setup('.circle_player',{
              controls : [
...
player.play();
  ...
  $(".plyr--video .plyr__controls .plyr__controls__item").show();
});
         player.on('exitfullscreen', (event) => {
  const instance = event.detail.plyr;
  $(".plyr--video .plyr__controls .plyr__controls__item").hide();
});
});
}
plyr();
          <?php if(Bsoptions('Emoji') == false) :?>
          $.getScript(setting.import+"assets/vendors/bs-emoji/bs-emoji.js?v=1.5",function(){
        $('#facecircle').BearsimpleEmoji($('.emotion'));
          $('#facecirclecomment').BearsimpleEmoji($('.emotion_circle'));
...
$("#friendcircle-comment-textarea").on("input propertychange",function(){
    ...
    $("#friendcircle-comment-submit").removeClass("friendcircle-comment-readySubmit");    
        ...
        error: function(req, status, error) {
         ...
         $('#friendcircle-comment-cancelsubmit').on('click',function(event){
             var response = dom('friendcircle-comment-container');
              event.stopPropagation();
              response.style.display = "none";
              $('.circle-action').css("pointer-events","auto");
         });
        $('.circle-comment-children').on('click',function(){
            $('.circle-action').css("pointer-events","none");
              var response = dom('friendcircle-comment-container');
          ...
          $('.circle-comment').on('click',function(){
              $('.circle-action').css("pointer-events","none");
              var response = dom('friendcircle-comment-container');
              var com = dom('comment-'+$(this).attr('data-coid'));
              window.localStorage.setItem('reply-coid', $(this).attr('data-coid'));
         ...
         $('#friendcircle-comment-submit').on('click',function(){
             ...
             $("#friendcircle-comment-submit").prop("disabled", true);
             $("#circleLoad").fadeIn();
             if(!$('#nologinshow').is(':hidden')){
             if(!$('#friendcircle-comment-nickname').val()){
				...
				$("#circleLoad").fadeOut();
				$("#friendcircle-comment-submit").prop("disabled", false);
				return false;
             };
...
if (preg_match("/^([0-9]+)([a-z]{1,2})$/i", $phpMaxFilesize, $matches)) {
    ...
    ?>
       ...
       function fileUploadError (error) {
        ...
        switch (code) {
            ...
            default:
                word = '<?php _e("上传出现错误，可能的原因：1、允许上传的文件类型与所上传的文件类型不符。2、配置了外部存储但没有正确配置。"); ?>';
                break;
        }
        alert(word);
            ...
            filters         :   {
				...
				init: {
					...
					$("#btn_v").hide();
						if (up.files.length > 9) {
							...
							plupload.each(files, function(file) { //遍历文件
								li += "<li id='" + file['id'] + "'><div class='fprogress'><span class='fbar'></span><span class='fpercent'>0%</span></div></li>";
							});
							$("#ul_pics").append(li);
							uploader.start();
						}
					...
					UploadProgress: function(up, file) { 
						...
						$("#" + file.id).find('.fbar').css({
							"width": percent + "%"
						});
						$("#" + file.id).find(".fpercent").text(percent + "%");
					},
					...
					Error: function(up, err) {
						fileUploadError.call(uploader, err);
					}
				}
			});
			uploader.init();
	...
	var uploader_video = new plupload.Uploader({
            ...
            filters         :   {
                max_file_size       :   '<?php echo $phpMaxFilesize;?>',
                mime_types          :   [{'title' : '允许上传的文件', 'extensions' : 'mp3,mp4,mov,wmv,wma,rmvb,rm,avi,flv,ogg,oga,ogv,flac,amr,speex'}],
                prevent_duplicates  :   false
            },
				multi_selection: false, 
				init: {
					...
					FileUploaded: function(up, file, info) {
					    if (200 == info.status) {
	                      ...
	                      //var data = eval("(" + info.response + ")");
	                      if (data) {
                         if(data[0].search('.mp4') !== -1){
                             $("#" + file.id).html("<div class='friendcircle_video'><video class='circle_player' controls crossorigin playsinline><source src='" + data[0] + "'></video><input name='image' value='" + data[0] + "' style='display:none'></div>");
                         }
                         else if(data[0].search('.amr') !== -1){
                             
                             $("#" + file.id).html("<div class=\"circle_player\"><div id=\"amr-begin-uploadPreview\" class=\"ui small label amr-begin\" data-circleid=\"uploadPreview\" data-amrfile='" + data[0] + "'>加载</div><div id=\"amr-play-uploadPreview\" class=\"ui small green icon label\" style=\"display:none\" disabled><i class=\"play icon\"></i></div><div id=\"amr-stop-uploadPreview\" class=\"ui small red icon label\" style=\"display:none\" disabled><i class=\"stop icon\"></i></div><input id=\"amr-progress-uploadPreview\" type=\"range\" min=\"0\" max=\"1\" step=\"any\" value=\"0\"  class=\"ui range\" disabled><label for=\"amr-progress-uploadPreview\" style=\"vertical-align:middle\"><span id=\"amr-cur-uploadPreview\">0'</span><span>/</span><span id=\"amr-duration-uploadPreview\">0'</span></label></div><input name='image' value='" + data[0] + "' style='display:none'></div>");
                         ...
                         $("#" + file.id).html("<div class='friendcircle_video'><audio class='circle_player' controls><source src='" + data[0] + "'></audio><input name='image' value='" + data[0] + "' style='display:none'></div>");  
                         }
						plyr();
						return;
	                      }
					    }
					    fileUploadError.call(uploader_video, {
                        code : plupload.HTTP_ERROR,
                        file : file
                    });
					},
					Error: function(up, err) {
						fileUploadError.call(uploader_video, err);
					}
				}
			});
			uploader_video.init();		
...
$("#commentform").css("pointer-events","auto");
 ...
 <?php if(Bsoptions('fcircle_hiddenMode') == false || Bsoptions('fcircle_hiddenMode') == ''):?>
