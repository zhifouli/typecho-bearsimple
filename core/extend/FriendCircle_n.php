...
?>
        ...
        <?php 
        $keys = array("mp3","mp4","mov","wmv","wma","rmvb","rm","avi","flv","ogg","oga","ogv","flac",'amr','speex');
        $resources = json_decode(self::circle_getInfo($this)['resources'],true);
        $keyword = parse_url(pathinfo($resources[0], PATHINFO_EXTENSION))['path'];
        $searchIfVideo = false;
        $type = 'video';
        foreach($keys as $key){
           if ($keyword == $key) { 
...
<div class="circle-pictures"  style="grid-template-columns:1fr 1fr 1fr;">
        <?php
