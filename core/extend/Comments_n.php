...
$url = 'https://ip.zxinc.org/api.php?type=json&ip='.$ip;
 ...
 public static function getLocation($comments)
    {
        $databaseFile = __DIR__.'/vendor/maxmind-db/GeoLite2-City.mmdb';
        $db = \Typecho\Db::get();
        $prefix = $db->getPrefix();
        $coid = $comments->coid;
        $data = $db->fetchRow($db->select()->from('table.bscore_iplocation_data')->where('coid = ?', $coid));
        if(!$data['location']){
    ...
    $ipAddress = $comments->ip;
    $record = $reader->get($ipAddress);
    $location = ($record['country']['names']['zh-CN'] ?? '未知').($record['subdivisions'][0]['names']['zh-CN'] ?? '').($record['city']['names']['zh-CN'] ?? ''); 
    $reader->close();
        }
        }
        else{
...
?>">
    ...
    <div class="comment"<?php if(Bsoptions('avatar__choose') == 'closeavatar'):?> style="margin-left:10px"<?php endif; ?>>
        <?php if(Bsoptions('avatar__choose') !== 'closeavatar'):?>
<a class="avatar" style="margin-top:0.5px">
      <img <?php echo lazyload('auto');?>src="<?php echo imgravatarq($this->mail); ?>">
    </a>
<?php endif; ?>
    ...
    <div class="content">
      ...
      <div class="text">
          ...
          <?php else:?>
          <div class="<?php if(Bsoptions('avatar__choose') !== 'closeavatar'):?>comment-excerpta<?php else:?>comment-excerptb<?php endif; ?> break"><?php echo reEmo($this->content,'comment'); ?> </div>
          <?php endif; ?>
       
       <?php if(Bsoptions('emaction_Openplace') && in_array('1',Bsoptions('emaction_Openplace'))):?>  
       <emoji-reaction theme="light" endpoint="<?php getEmactionAction();?>" reacttargetid="comment-<?php echo $this->coid;?>" style="flex-wrap: nowrap; max-width: 100%; display: flex; gap: 0.375rem;height: 1.5rem;<?php if(Bsoptions('avatar__choose') !== 'closeavatar'):?>margin-left:-40px;<?php else:?>margin-top:10px;<?php endif;?>margin-bottom:10px"></emoji-reaction> 
       <?php endif;?>
       </div> 
      
      
      <div class="actions"<?php if(Bsoptions('avatar__choose') !== 'closeavatar'):?> style="margin-left:-40px"<?php endif; ?>>
...
<?php
    ...
    private function getParent(){
        ...
        if($parentID=='0'){ return '';}
            ...
            return '  <i class="at icon" style="margin:auto"></i><a href="#comment-'.$parentID.'"> '.$author['author'].'</a> ';
        }
    }  
