...
if(!$cover && $options['Poster__AttChoose'] == true){
	  $thumb = $options['Poster__AttUrl'];   
	}	 
	return $thumb;
}
...
function getDescription($var)
    {
          if($var->is('post')){
              ...
              if($var->fields->excerpt){
              echo $var->fields->excerpt;    
              }
              else{
                  
             if(excerpt($var->excerpt,50) == ''){
                  echo getTitle($var);    
                  }
                  else{
           echo excerpt($var->excerpt,50);
                  }
...
echo '
<div class="barticle-item">
      <a href="'.$permalink.'" class="barticle-link">
        <h4 class="barticle-title">'.$post_title.'</h4>
      </a>
      <div class="barticle-excerpt">
        '.$targetSummary.'
      </div>
      <div class="barticle-meta">
        <span class="bmeta-item"><i class="fas fa-calendar-alt"></i> '.$date.'</span>
      </div>
    </div>';
}
$i++;
...
function historyToday($created)
{
    ...
    $sql = "SELECT * FROM {$prefix}contents WHERE to_char(created, 'M/D') = '{$date}' and created <= {$time} and created != {$created} and type = 'post' and status = 'publish' and (password is NULL or password = '') LIMIT ".$limit;
     }
...
$avatar = imgravatarq($th->author->mail);
            return <<<EOF
<div class="reading-mode-overlay bg-light" id="reading-mode-overlay">
  <div class="reading-mode-content">
    <!-- 关闭阅读模式按钮 -->
    <button class="action-btn exit-reading-mode-btn" id="exit-reading-mode">
      <i class="fas fa-times"></i> 关闭阅读模式
    </button>

    <!-- 文章标题 -->
    <h1 class="reading-article-title">{$title}</h1>
    ...
    <!-- 阅读设置按钮 -->
    <button class="action-btn reading-settings-btn" id="toggle-settings-panel">
      <i class="fas fa-cog"></i> 阅读选项
    </button>

    <!-- 阅读模式设置面板 -->
    <div class="reading-mode-panel" id="settings-panel">
      <h3>阅读选项</h3>
      <div class="options">
        <button id="bg-light"><i class="fas fa-sun"></i> 浅色背景</button>
        <button id="bg-sepia"><i class="fas fa-book"></i> 护眼模式</button>
        <button id="bg-dark"><i class="fas fa-moon"></i> 深色模式</button>
        <div style="height: 10px;"></div>
        <button id="text-small"><i class="fas fa-text-height"></i> 小字体</button>
        <button id="text-medium"><i class="fas fa-text-height"></i> 中字体</button>
        <button id="text-large"><i class="fas fa-text-height"></i> 大字体</button>
      </div>
    </div>
...
';
    ...
    if($options['article_textindent'] == true){
    echo '.post-content p,
.reading-mode p{
  text-indent: 2em;
}
...
echo '<div class="ui top left dropdown button" onclick="isHidden(\'div_reward\')">
       <i class="gift violet icon"></i>支持作者</div>';
}
else{
   $linkUrl = 'window.open("'.$options['RewardOpenPaypalText'].'","_blank")';
  echo'
  <div class="ui top left button" onclick="window.__DONATE_INSTANCE?.show()">
       <i class="gift violet icon"></i>支持作者';
    echo'
</div>';   
}
...
//自动分页方法
function autoPage($array,$pagesize,$current){
    if(!$array){
        return;
    }
  $_return = array();
  ...
  $page .= '<a class="ui large label" href="?page='.$prev.'">上一页</a>';
  for($i=1;$i<=$total;$i++){
   
    if($i==$current){
      $page .= '<a href="?page='.$i.'" class="ui black large label">'.$i.'</a>';
    }
    else{
         $page .= '<a href="?page='.$i.'" class="ui large label">'.$i.'</a>';
    }
  }
  $page .= '<a class="ui large label" href="?page='.$next.'">下一页</a>';
  $page .= '<a class="ui large label" href="?page='.$total.'">尾页</a></div></div>';
  $pagearray["page"] = $page;
  if($total == 1){
  $pagearray["page"] = '';    
  }
...
if($post){
      ...
      echo '<div class="recent-comments-text">'.$comment_text.'</p>
                        </div>
                       ';    
      }
      }
     ...
     echo $post_url.'<div class="recent-comments-meta">'.date('Y-m-d',$comment['created']).'</div></li>';
    }
    echo '</ul></div>';
}
