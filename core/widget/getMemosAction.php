...
if($_POST['action'] == 'getmemo'){
if(Bsoptions('emaction_Openplace') && in_array('3',Bsoptions('emaction_Openplace')) && Bsoptions('emaction_Closelike') == true){
       $emaction = true;
   }
   else{
       $emaction = false;
...
if(trim($temoptions['memos_Username']) !== ''){
 $memos_all = memos_getRequest($temoptions['memos_Url'].'/api/v1/memo/stats?creatorUsername='.$temoptions['memos_Username'],'GET');
$memos = memos_getRequest($temoptions['memos_Url'].'/api/v1/memo?limit='.$_POST['number'].'&creatorUsername='.$temoptions['memos_Username'],'GET');  
}
else{
...
$data = [];
        foreach($val['resourceList'] as $v){
            ...
            if(!$v['externalLink']){
            $data[] = '<img data-fancybox="image" data-caption="'.$v['filename'].'" '.lazyload('auto').'src="'.$temoptions['memos_Url'].'/o/r/'.$v['id'].'" referrerPolicy="no-referrer" data-memo-image>';  
            }
            else{
            $data[] = '<img data-fancybox="image" data-caption="'.$v['filename'].'" '.lazyload('auto').'src="'.$v['externalLink'].'" referrerPolicy="no-referrer" data-memo-image>';      
            }
            }
            //反之则当做超链接处理
            else{
...
if (strpos($content, '<h1') !== false) {
        $content = preg_replace('/\<h1\>(.*?)\<\/h1\>/sm', '<a class="ui label" style="margin-top:10px"><i class="tag icon"></i>$1</a>', $content);
   }
   
 $replacement = '
  <img data-fancybox="image" data-caption="$2" '.$lazyload.'src="$1">'; 
