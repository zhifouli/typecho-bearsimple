...
if(!$response->result){
    ...
    if(empty($_POST['friendpic'])){
    if(empty(Bsoptions('FriendLinkLogoDefault'))){
    $_POST['friendpic'] = $options->siteUrl.'/usr/themes/bearsimple/assets/images/default-logo.png';
    }
    else{
    $_POST['friendpic'] = Bsoptions('FriendLinkLogoDefault');
    }
    }
    $opts = array(
  'http'=>array(
    ...
    $db= \Typecho\Db::get();
        $link = array(
            ...
            if($temoptions['FriendLinkSubmitSendMail'] == true && $temoptions['Smtp_open'] == true){
             ...
             file_get_contents(Common::url('/action/' . BsCore_Plugin::$action, Options::alloc()->index).'?do=linkMail&type=notifyaccept'.'&friendname='.urlencode($_POST['friendname']),false,$context);   
            }
            $this->response->throwJson([
            'code'=> 1,
            'msg' => 'success',
