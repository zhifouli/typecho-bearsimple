...
header("HTTP/1.1 200 OK");
    ...
    if (strpos($_SERVER['HTTP_REFERER'], str_replace($removeChar, "", $options->siteUrl)) !== false && $user->hasLogin() && $user->pass('administrator', true)) {   
        ...
        if($temoptions['FriendLinkAcceptSendMail'] == true && $temoptions['Smtp_open'] == true && $_POST['linkmail'] !== '' && $_POST['linkmail'] !== '无'){
            //审核通过发送邮件
            file_get_contents(Common::url('/action/' . BsCore_Plugin::$action, Options::alloc()->index).'?do=linkMail&type=notifysuccess&mail='.trim($_POST['linkmail']).'&friendname='.urlencode($_POST['linkname']));   
        }
...
$result['list'] = $links;       
        ...
        if($temoptions['FriendLinkAcceptSendMail'] == true && $temoptions['Smtp_open'] == true && $_POST['linkmail'] !== '' && $_POST['linkmail'] !== '无'){
         //审核不通过发送邮件   
         file_get_contents(Common::url('/action/' . BsCore_Plugin::$action, Options::alloc()->index).'?do=linkMail&type=notifyfail&mail='.trim($_POST['linkmail']).'&friendname='.urlencode($_POST['linkname'])); 
        }
        $links = $db->fetchAll($db->select()->from('table.bscore_friendlinks')->where('status = ?', 'waiting')->order('id',Typecho\Db::SORT_DESC));
        $result = array(
    'list' => array()
