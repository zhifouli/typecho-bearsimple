...
function syncDb(){
    ...
    $table5 = $db->getPrefix() . 'bscore_wechat_msgids';
...
//获取豆瓣
        if (strpos($_SERVER['REQUEST_URI'], 'getDouban') !== false) {
            $self->response->setStatus(200);
            $self->setThemeFile("core/widget/getDouban.php");
        }
//获取Bangumi
        if (strpos($_SERVER['REQUEST_URI'], 'getBangumi') !== false) {
            $self->response->setStatus(200);
            $self->setThemeFile("core/widget/getBangumi.php");
        }
//获取标签
        if (strpos($_SERVER['REQUEST_URI'], 'getTags') !== false) {
            $self->response->setStatus(200);
            $self->setThemeFile("core/widget/getTags.php");
        }
