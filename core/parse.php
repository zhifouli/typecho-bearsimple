...
function quotebspaperCallback($matches){
    ...
    $week = $arr[date("w")];
   $news = '';
$data = Curl::ua('android')->get('https://top.baidu.com/board')->html();
$data = json_decode(str_Intercept($data, '<!--s-data:', '-->'), true);
for($i=0;$i!=10;$i++)
{
$new = ($i+1)."、".$data['cards'][0]['content'][$i]['query'] . '  ' . str_replace(' ','',$data['cards'][0]['content'][$i]['desc']);
...
function parseGithubCallback($matches)
    {
        ...
        return <<<EOF
        <div class="ui relaxed divided list" style="margin:auto"><div class="item"><i class="big github middle aligned icon"></i><div class="content"><a class="header" href="{$result['html_url']}" title="{$result['description']}" target="_blank">{$result['full_name']}</a><div class="description">{$star} {$result['pushed_at']}</div></div></div></div>
EOF;
    }
...
function parseMp3Callback($matches)
    {
        ...
        if ($matches[4] != '/' && $matches[5]){
            $regex = get_shortcode_regex(array('mp3'));
            ...
            if ( ( false !== strpos( $content, '[' ) ) && preg_match_all("/$regex/", $content , $all)){
                foreach ($all[0] as $k=>$v){
                    $a = shortcode_parse_atts($all[3][$k]);
                    $r = parsebsMp3MultiFunc(trim($all[5][$k]), $a);
...
EOF;
    ...
    function parsebsMp3MultiFunc($content = '',$atts = array())
    {
        $content = strip_tags($content);
        $return = array();
     
        $atts['lyric'] = "false";
            if (isset($atts['singer'])) {
                $atts['singer'] = $atts['singer'];
            ...
            $atts['author'] = $atts['singer'];
            $atts['lrc'] = "[00:00.00]\u627e\u4e0d\u5230\u6b4c\u8bcd\n[99:00.00] ";
      unset($atts['singer']);
      unset($atts['name']);
      unset($atts['image']);
                $return[] = $atts;
...
//KBD回调
function parseKbdCallback($matches)
{
    $attr = htmlspecialchars_decode($matches[3]);
        $attrs = shortcode_parse_atts($attr);
        $style = 'ui label';
        if($attrs){
            if(@$attrs['style'] == 'dark'){
            $style = 'ui inverted label';
        }
        elseif(@$attrs['style'] == 'white'){
            $style = 'ui basic label';
        }
...
function parseTimePlanCallback($matches){
    ...
    $attr = htmlspecialchars_decode($matches[3]);
        ...
        $matches[5] = preg_replace("/\[bstime time=\"(.*?)\"\](.*?)\[\/bstime\]/sm",'<tr><td data-label="时间">$1</td><td data-label="内容">$2</td></tr>',$matches[5]);
        $matches[5] = str_replace("<br />",'',$matches[5]);
        $matches[5] = preg_replace("/\<br>/sm",'',$matches[5]);
    return <<<EOF
    <div class="schedule-container">
        <div class="schedule-title">{$attrs['title']}</div>
...
function ParseAboutMe($type = null){
    $options = bsOptions::getInstance()::get_option( 'bearsimple' );
    if($type == 'plan'){
     $content = $options['aboutme_gh'];   
    }
    else{
...
function ShortCodePage($post,$title,$remember,$cid,$login,$articletype,$modetype='noreadmode'){
 ...
 $replacement = '
<img data-fancybox="image" data-caption="$2" class="bearmark lazyload lazyload-style-'.$options['Lazyload_style'].'" '.lazyload(1).'src="$1"   alt="'.$title.'" title="点击放大图片">'.$alt; 
    ...
    if (strpos($content, '[bskbd') !== false) {
            $pattern = get_shortcode_regex(array('bskbd'));
            $content = preg_replace_callback("/$pattern/", 'parseKbdCallback', $content);
    }
    //tabs
        ...
        $content = preg_replace_callback("/<a href=\"(.*?)\">(.*?)<\/a>/is", function ($matches){
           ...
           $optionss = Helper::options();
            if (strpos($matches[1],$optionss->siteUrl) !== false || strpos(substr($matches[1],0,6),"http") === false){
                return '<a class="link-post"  href="'.$matches[1].'"><i class="fas fa-external-link-alt"></i>'.$matches[2].'</a>';
            }else{
                if ($options['Link_blank']== '2'){
                return '<a class="link-post"  href="'.$matches[1].'" target="_blank"><i class="fas fa-external-link-alt"></i>'.$matches[2]."</a>";
                }
                else{
                return '<a class="link-post" href="'.$matches[1].'"><i class="fas fa-external-link-alt"></i>'.$matches[2]."</a>";     
                }
 ...
 <div class="masonry-brick">
      <img data-fancybox="image" data-caption="$2" '.lazyload(1).'src="$1" class="'.$mark.' lazyload lazyload-style-'.$options['Lazyload_style'].'"   alt="$2" title="点击放大图片"></div>
   '; 
    $content = preg_replace($pattern, $replacement, $content);
 ...
 <div class="masonry-brick">
   ...
   if($articletype == 'pic_Mode'){
    $content = "<div class=\"masonry-container\"><div class=\"masonry-columns\">".preg_replace("/<p.*?>|<\/p>|<br.*?>/is","", $content)."</div></div>";
   }
   if($modetype == 'noreadmode'){
...
function ShortCode($post,$t,$login,$articletype,$modetype='noreadmode'){
 ...
 $replacement = '
<img data-fancybox="image" data-caption="$2" class="pure-img '.$mark.' lazyload lazyload-style-'.$options['Lazyload_style'].'" '.lazyload(1).'src="$1"  alt="$2" title="点击放大图片">'.$alt; 
    $content = preg_replace($pattern, $replacement, $content);
...
function getCircleAction(){
    $getCircleAction = Helper::options()->siteUrl.'index.php/getCircleAction';
    echo $getCircleAction;
}
function getEmactionAction(){
    $getEmactionAction = Helper::options()->siteUrl.'index.php/getEmaction';
    echo $getEmactionAction;
}
function getAiTools(){
    $getAiTools = Helper::options()->siteUrl.'index.php/getAiTools';
    echo $getAiTools;
}
function getTagsApi(){
    $getTags = Helper::options()->siteUrl.'index.php/getTags';
    echo $getTags;
}