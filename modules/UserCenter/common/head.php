...
<?php endif; ?>
 ...
 <title>用户中心 - <?php echo $this->options->title;?></title>
    <meta name="description" content="用户中心 - <?php echo $this->options->title;?>" />
    <meta property="og:title" content="用户中心 - <?php echo $this->options->title;?>" />
    <meta property="og:description" content="用户中心 - <?php echo $this->options->title;?>" />
...
<link href="<?php AssetsDir();?>assets/vendors/usercenter/usercenter.min.css?v=<?php echo themeVersion(); ?>" rel="stylesheet">
...
<link href="<?php AssetsDir();?>assets/vendors/bs-announcement/bs-announcement.min.css?v=1" rel="stylesheet" type="text/css">
<?php endif; ?>
...
<?php endif; ?>
<?php endif; ?>
        <?php if(is_array(Bsoptions('Menu'))):?>
  <?php foreach (Bsoptions('Menu') as $item): 
    $menuText = $item['Menu_Text'];
    $needLogin = !empty($item['Menu_LoginShow']);
    if (empty(trim($menuText['url'])) && empty(trim($menuText['text']))) {
        continue;
    }
    $url = !empty(trim($menuText['url'])) ? htmlspecialchars(trim($menuText['url'])) : 'javascript:;';
    $text = !empty(trim($menuText['text'])) ? htmlspecialchars(trim($menuText['text'])) : '未命名导航';
?>
<li class="<?= $needLogin ? 'need-login' : '' ?>" 
    <?= $needLogin ? 'style="display:none"' : '' ?>>
    <a href="<?= $url ?>" 
       title="<?= $text ?>"
       <?= ($menuText['target'] === '_blank') ? 'target="_blank"' : '' ?>>
       <?= $text ?>
    </a>
</li>
<?php endforeach; ?>
...
<?php endif;?>
  ...
  <?php foreach (Bsoptions('Menu') as $item): 
    ...
    $text = !empty(trim($menuText['text'])) ? htmlspecialchars(trim($menuText['text'])) : '未命名导航';
?>
<li class="sm-nav-item<?= $needLogin ? ' need-login' : '' ?>" 
    <?= $needLogin ? 'style="display:none"' : '' ?>>
    <a class="sm-nav-link" href="<?= $url ?>" 
       title="<?= $text ?>"
       <?= ($menuText['target'] === '_blank') ? 'target="_blank"' : '' ?>>
       <?= $text ?>
    </a>
