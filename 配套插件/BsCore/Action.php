   ...
   public function bsshortlink()
    {
        ...
        if ($requestString === \BsCore\Plugin::urlSafeB64Encodes(\BsCore\Plugin::urlSafeB64Decodes($requestString))) {
            // 自动转换链接处理
            $target = \BsCore\Plugin::urlSafeB64Decodes($requestString);
            $allow_redirect = false; // 默认不允许跳转
            // 检查 referer
            $allow_redirect = \BsCore\Plugin::checkDomains($referer, $refererList);
        ...
        }elseif(\BsCore\Plugin::bCore_getDiyLink($requestString)['url']) {
               $target = \BsCore\Plugin::bCore_getDiyLink($requestString)['url'];
        }else {
    ...
    public function clean_cache()
    {
        ...
        if ($options['Cache_choose'] == 'memcached' || $options['Cache_choose'] == 'redis' || $options['Cache_choose'] == 'apcu' || $options['Cache_choose'] == 'sqlite'|| $options['Cache_choose'] == 'memory'|| $options['Cache_choose'] == 'flysystem') {
            \BsCore\Plugin::initBackend($options['Cache_choose']);
            try {
                \BsCore\Plugin::$cache->flush();
                $data['code']  = '1';
	            $data['message']  = '缓存清理成功!';
    ...
    { 
        ...
        //发件人邮箱
        $this->_email->from = $options['CommentNotify__SenderForm'] ? $options['CommentNotify__SenderForm'] : $options['CommentNotify__Username'];
        ...
        if (
            ...
            if (
                ...
                $this->_email->originalText   = \BsCore\Plugin::getEmo($original['text']);
...
$sysoptions = \Typecho\Widget::widget('Widget_Options');
        ...
        $this->_email->toName = $this->_user->screenName;
        $this->_email->subject = '您的站点有新的友链申请等待审核哦';
        $this->_email->altBody = '友链审核提醒';
        $this->_email->msgHtml = str_replace($search, $replace, file_get_contents(dirname(__FILE__)."/modules/MailServices/friendlink_notify.html"));
        //$this->_email->msgHtml = '主人，有人提交了友链申请啦，快去审核下叭～';
        break;
        case 'notifysuccess':
        $this->_email->to = $_GET['mail'];
        $this->_email->toName = $_GET['mail'];
        $this->_email->subject = '友链审核结果';
        $this->_email->altBody = '友链审核结果';
        $this->_email->msgHtml = str_replace($search, $replace, file_get_contents(dirname(__FILE__)."/modules/MailServices/friendlink_approved.html"));
        ...
        $this->_email->subject = '友链审核结果';
         $this->_email->altBody = '友链审核结果';
        $this->_email->msgHtml = str_replace($search, $replace, file_get_contents(dirname(__FILE__)."/modules/MailServices/friendlink_reject.html"));
