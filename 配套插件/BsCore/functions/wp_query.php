    ...
    class WP_Query
    {
        ...
        public function parse_query($query = '')
        {
            ...
            if (('' !== $qv['attachment']) || !empty($qv['attachment_id'])) {
                ...
                // there is no tax in typecho, so we don't concern it
...
//            unset( $tax_query );
            ...
            if (!empty($qv['status'])) {
                if (is_array($qv['status'])) {
                    $qv['status'] = array_map('sanitize_key', $qv['status']);
                } else {
                    $qv['status'] = preg_replace('|[^a-z0-9_,-]|', '', $qv['status']);
                }
            }
...
//        do_action( 'parse_tax_query', $this );
        ...
        {
            $wpdb = \Typecho\Db::get();
            $wpdb->posts = $wpdb->getPrefix() . 'contents';
            $search = '';
            ...
            foreach ($q['search_terms'] as $term) {
                ...
                if ($n && !$exclude) {
                    $like = '%' . esc_like($term) . '%';
                    $q['search_orderby_title'][] = "{$wpdb->posts}.title LIKE '$like'";
                }

                $like = $n . esc_like($term) . $n;
                $search .= sprintf("{$searchand}({$wpdb->posts}.title $like_op '%s' $andor_op {$wpdb->posts}.text $like_op '%s')", $like, $like);
                $searchand = ' AND ';
            }
        ...
        protected function parse_search_order(&$q)
        {
            ...
            if ($q['search_terms_count'] > 1) {
                ...
                if ($like) {
                    $search_orderby .= sprintf("WHEN {$wpdb->posts}.title LIKE %s THEN 1 ", $like);
                }
                ...
                // Sentence match in 'text' and 'post_excerpt'.
                if ($like) {
                    $search_orderby .= sprintf("WHEN {$wpdb->posts}.text LIKE %s THEN 5 ", $like);
                }
...
//        $this->meta_query->parse_query_vars( $q );
            ...
            switch ($q['fields']) {
                case 'ids':
                    $fields = "{$wpdb->getPrefix()}contents.ID";
                    break;
//            case 'id=>parent':
//                $fields = "{$db->getPrefix()}contents.ID, {$wpdb->posts}.post_parent";
//                break;
                default:
                    $fields = "{$wpdb->getPrefix()}contents.*";
            }
...
//            $q['name'] = sanitize_title_for_query( $q['name'] );
            ...
            if (is_numeric($q['post_parent'])) {
                $where .= sprintf(" AND {$wpdb->posts}.post_parent = %d ", $q['post_parent']);
            ...
            if (isset($q['comment_count'])) {
                ...
                if (isset($q['comment_count']['value'])) {
                    ...
                    $where .= sprintf(" AND {$wpdb->posts}.comment_count {$q['comment_count']['compare']} %d", $q['comment_count']['value']);
                }
            }
...
//        $where .= $search . $whichauthor . $whichmimetype;
             ...
             */
                if (isset($q['orderby']) && (is_array($q['orderby']) || false === $q['orderby'])) {
                    ...
                    $orderby = "{$wpdb->getPrefix()}contents.created " . $q['order'];
                }
...
//        }
            ...
            if (isset($q['post_password'])) {
                $where .= sprintf(" AND {$wpdb->posts}.post_password = %s", $q['post_password']);
                if (empty($q['perm'])) {
                    $q['perm'] = 'readable';
                }
            ...
            if (!empty($q['comment_status'])) {
                $where .= sprintf(" AND {$wpdb->posts}.comment_status = %s ", $q['comment_status']);
            }

            if (!empty($q['ping_status'])) {
                $where .= sprintf(" AND {$wpdb->posts}.ping_status = %s ", $q['ping_status']);
            }
            ...
            if ('any' === $post_type) {
                ...
                $post_type_where = sprintf(" AND {$wpdb->getPrefix()}contents.post_type = '%s'", $post_type);
                $post_type_object = get_post_type_object($post_type);
            ...
            if ($skip_post_status) {
                ...
                if (in_array('any', $q_status, true)) {
                    foreach (get_post_stati(array('exclude_from_search' => true)) as $status) {
                        if (!in_array($status, $q_status, true)) {
                            $e_status[] = "{$wpdb->getPrefix()}contents.status <> '$status'";
                        }
                    }
                } else {
                    ...
                    foreach (get_post_stati() as $status) {
                        if (in_array($status, $q_status, true)) {
                            if ('private' === $status) {
                                $p_status[] = "{$wpdb->getPrefix()}contents.status = '$status'";
                            } else {
                                $r_status[] = "{$wpdb->getPrefix()}contents.status = '$status'";
                            }
                        }
                    }
                }
                ...
                if ($post_status_join) {
                    ...
                    foreach ($statuswheres as $index => $statuswhere) {
                        $statuswheres[$index] = "($statuswhere OR ({$wpdb->getPrefix()}contents.status = 'inherit' AND " . str_replace($wpdb->posts, 'p2', $statuswhere) . '))';
                    }
                }
                ...
                if (!empty($queried_post_types)) {
                    ...
                    foreach ($queried_post_types as $queried_post_type) {
                        ...
                        $type_where = '(' . "{$wpdb->getPrefix()}contents.post_type = '$queried_post_type' AND (";

                        // Public statuses.
                        $public_statuses = get_post_stati(array('public' => true));
                        $status_clauses = array();
                        foreach ($public_statuses as $public_status) {
                            $status_clauses[] = "{$wpdb->getPrefix()}contents.status = '$public_status'";
                        }
...
//                            $type_where .= " OR {$wpdb->getPrefix()}contents.status = '$admin_all_status'";
//                        }
//                    }
...
//                $cwhere   = "WHERE ( status = 'publish' OR ( status = 'inherit' AND post_type = 'attachment' ) ) AND comment_approved = '1'";
//                $cgroupby = '';
//            }
//
//
...
//        }
            ...
            if (null === $this->posts) {
                $split_the_query = ($old_request == $this->request && "{$wpdb->getPrefix()}contents.*" === $fields && !empty($limits) && $q['posts_per_page'] < 500);
                ...
                if ($split_the_query) {
                    ...
                    $this->request = "SELECT {$wpdb->getPrefix()}contents.cid FROM {$wpdb->getPrefix()}contents  WHERE 1=1 $where $groupby $orderby";
                    //echo "SELECT $found_rows $distinct {$wpdb->getPrefix()}contents.cid FROM {$wpdb->getPrefix()}contents  WHERE 1=1 $where $groupby $orderby $limits";
                    ...
                    $this->request = str_replace('post_type', 'type', $this->request);

                    $ids = $wpdb->fetchAll($this->request);
...
//                    _prime_post_caches( $ids, $q['update_post_term_cache'], $q['update_post_meta_cache'] );
                    ...
                    $this->posts = $wpdb->fetchAll($this->request);
            ...
            if (!empty($this->posts) && ($this->is_single || $this->is_page)) {
                ...
                // If the status was specifically requested, let it pass through.
                if (!in_array($status, $q_status, true)) {
...
//            do_action( 'loop_no_results', $this );
        ...
        public function get_queried_object()
        {
            ...
            if ($this->is_category || $this->is_tag || $this->is_tax) {
                if ($this->is_category) {
                    ...
                    if ($this->get('tag_id')) {
                        $term = get_term($this->get('tag_id'), 'tag');
                    } elseif ($this->get('tag')) {
                        $term = get_term_by('slug', $this->get('tag'), 'tag');
                    }
                } else {
        ...
        public function is_attachment($attachment = '')
        {
            ...
            if (in_array((string)$post_obj->ID, $attachment, true)) {
                return true;
            } elseif (in_array($post_obj->title, $attachment, true)) {
                return true;
         ...
         *
         * Otherwise the same as @return bool Whether the query is for the front page of the site.
         * @since 3.1.0
         *
        ...
        public function is_page($page = '')
        {
            ...
            if (in_array((string)$page_obj->ID, $page, true)) {
                return true;
            } elseif (in_array($page_obj->title, $page, true)) {
                return true;
        ...
        public function is_single($post = '')
        {
            ...
            if (in_array((string)$post_obj->ID, $post, true)) {
                return true;
            } elseif (in_array($post_obj->title, $post, true)) {
                return true;
