...
use CaptchaClient;
require_once 'modules/Markdown/Parser.php';
require_once 'modules/Tinypng/Tinify/Exception.php';
require_once 'modules/Tinypng/Tinify/ResultMeta.php';
require_once 'modules/Tinypng/Tinify/Result.php';
require_once 'modules/Tinypng/Tinify/Source.php';
require_once 'modules/Tinypng/Tinify/Client.php';
require_once 'modules/Tinypng/Tinify.php';
require_once 'modules/aliyun_oss/aliyun-oss-php-sdk-2.6.0.phar';
use OSS\OssClient;
use OSS\Core\OssException;
require_once 'modules/qiniu_kodo/autoload.php';
use Qiniu\Auth;
use Qiniu\Storage\BucketManager;
use Qiniu\Storage\UploadManager;
require_once 'modules/upyun_uss/vendor/autoload.php';
use Upyun\Upyun;
use Upyun\Config;
require_once 'modules/huawei_obs/vendor/autoload.php';
require_once 'modules/huawei_obs/obs-autoloader.php';
use Obs\ObsClient;
require_once 'modules/baidu_bos/BaiduBce.phar';
use BaiduBce\Services\Bos\BosClient;
use BaiduBce\Services\Bos\BosOptions;

require_once 'modules/cache/autoload.php';
/**
 * BearSimple主题核心插件
 * <br>安装后无需进行其他设置
 * @package BsCore
 * @author BearNotion
 * @version v2.9.4-release
...
class Plugin implements PluginInterface
{
  ...
  const PLUGIN_DIR = '/usr/plugins/BsCore/';
public static $cache = null;
    ...
    public static $_panel  = 'BsCore/pages/panel.php';
    ...
    public static function activate()
    {
         ...
         */
        bsRouter::initRouter();
        Helper::addPanel(0, self::$_panel, 'BearSimple主题扩展控制', 'BearSimple主题扩展控制面板', 'administrator');
        ...
        \Typecho\Plugin::factory('Widget_Feedback')->finishComment_134 = [__CLASS__, 'filter2'];
...
\Typecho\Plugin::factory('Widget_Upload')->uploadHandle = [__CLASS__, 'uploadHandle'];
   \Typecho\Plugin::factory('Widget_Upload')->modifyHandle = [__CLASS__, 'modifyHandle'];
    ...
    \Typecho\Plugin::factory('Widget_Archive')->beforeRender = [__CLASS__, 'Widget_Archive_beforeRender'];
        ...
        //阿里云COS
        if(get_option('bearsimple')['bucket_choose'] == '3'){
\Typecho\Plugin::factory('Widget_Upload')->uploadHandle = [__CLASS__, 'ali_uploadHandle'];
   \Typecho\Plugin::factory('Widget_Upload')->modifyHandle = [__CLASS__, 'ali_modifyHandle'];
    ...
    \Typecho\Plugin::factory('Widget_Upload')->attachmentDataHandle = [__CLASS__, 'ali_attachmentDataHandle'];
        ...
        //七牛云Kodo
        if(get_option('bearsimple')['bucket_choose'] == '4'){
\Typecho\Plugin::factory('Widget_Upload')->uploadHandle = [__CLASS__, 'qiniu_uploadHandle'];
   \Typecho\Plugin::factory('Widget_Upload')->modifyHandle = [__CLASS__, 'qiniu_modifyHandle'];
    \Typecho\Plugin::factory('Widget_Upload')->deleteHandle = [__CLASS__, 'qiniu_deleteHandle'];
    \Typecho\Plugin::factory('Widget_Upload')->attachmentHandle = [__CLASS__, 'qiniu_attachmentHandle'];
    \Typecho\Plugin::factory('Widget_Upload')->attachmentDataHandle = [__CLASS__, 'qiniu_attachmentDataHandle'];
        }
        //又拍云USS
        if(get_option('bearsimple')['bucket_choose'] == '5'){
\Typecho\Plugin::factory('Widget_Upload')->uploadHandle = [__CLASS__, 'upyun_uploadHandle'];
   \Typecho\Plugin::factory('Widget_Upload')->modifyHandle = [__CLASS__, 'upyun_modifyHandle'];
    \Typecho\Plugin::factory('Widget_Upload')->deleteHandle = [__CLASS__, 'upyun_deleteHandle'];
    \Typecho\Plugin::factory('Widget_Upload')->attachmentHandle = [__CLASS__, 'upyun_attachmentHandle'];
    \Typecho\Plugin::factory('Widget_Upload')->attachmentDataHandle = [__CLASS__, 'upyun_attachmentDataHandle'];
        }
        //华为云OBS
        if(get_option('bearsimple')['bucket_choose'] == '7'){
\Typecho\Plugin::factory('Widget_Upload')->uploadHandle = [__CLASS__, 'hw_uploadHandle'];
   \Typecho\Plugin::factory('Widget_Upload')->modifyHandle = [__CLASS__, 'hw_modifyHandle'];
    \Typecho\Plugin::factory('Widget_Upload')->deleteHandle = [__CLASS__, 'hw_deleteHandle'];
    \Typecho\Plugin::factory('Widget_Upload')->attachmentHandle = [__CLASS__, 'hw_attachmentHandle'];
    \Typecho\Plugin::factory('Widget_Upload')->attachmentDataHandle = [__CLASS__, 'hw_attachmentDataHandle'];
        }
        //百度云BOS
        if(get_option('bearsimple')['bucket_choose'] == '6'){
\Typecho\Plugin::factory('Widget_Upload')->uploadHandle = [__CLASS__, 'bd_uploadHandle'];
   \Typecho\Plugin::factory('Widget_Upload')->modifyHandle = [__CLASS__, 'bd_modifyHandle'];
    \Typecho\Plugin::factory('Widget_Upload')->deleteHandle = [__CLASS__, 'bd_deleteHandle'];
    \Typecho\Plugin::factory('Widget_Upload')->attachmentHandle = [__CLASS__, 'bd_attachmentHandle'];
    \Typecho\Plugin::factory('Widget_Upload')->attachmentDataHandle = [__CLASS__, 'bd_attachmentDataHandle'];
        }
        //代码高亮
       ...
       //\Typecho\Plugin::factory('admin/header.php')->header_1081 = [__CLASS__, 'loginverify'];
         ...
         \Typecho\Plugin::factory('Widget_Feedback')->finishComment_28654 = [__CLASS__,'parseComment'];
        \Typecho\Plugin::factory('Widget_Comments_Edit')->finishComment_25674 = [__CLASS__,'parseComment'];
        \Typecho\Plugin::factory('Widget_Comments_Edit')->mark_25544 = [__CLASS__,'parseCommentMark'];
         }
         ...
         if(get_option('bearsimple')['Tinypng_open'] == true && get_option('bearsimple')['bucket_choose'] !== '2'&& get_option('bearsimple')['bucket_choose'] !== '3'&& get_option('bearsimple')['bucket_choose'] !== '4'&& get_option('bearsimple')['bucket_choose'] !== '5'&& get_option('bearsimple')['bucket_choose'] !== '6'&& get_option('bearsimple')['bucket_choose'] !== '7'){
        ...
        \Typecho\Plugin::factory('admin/write-post.php')->bottom = [__CLASS__,'tinypng_bottom'];
         ...
         \Typecho\Plugin::factory('Widget_Archive')->beforeRender_siteMaintenance_15613 = [__CLASS__, 'siteMaintenance'];
        \CSF::activateEvent();
         ...
         */
         ...
         $table5 = $db->getPrefix() . 'bscore_memos_agree_data';
    $adapter = $db->getAdapterName();
        if ("Pdo_SQLite" === $adapter || "SQLite" === $adapter) {
            ...
            $db->query(" CREATE TABLE IF NOT EXISTS " . $table . " (
			   ...
			   //memos点赞表检测初始化
			   $db->query(" CREATE TABLE IF NOT EXISTS " . $table5 . " (
			   id INTEGER PRIMARY KEY,
			   memosid  TEXT,
			   agree TEXT)");
...
if (!array_key_exists('signature', $db->fetchRow($db->select()->from('table.users')))) {
$db->query('ALTER TABLE ' . $prefix . 'users ADD signature TEXT NOT NULL  DEFAULT 0;');
}

if (strpos($db->fetchAll($db->query("select sql from sqlite_master where type = 'table' and name = '" . $table . "'"))[0]['sql'], 'checkurl') == false) {
            $db->query('ALTER TABLE ' . $prefix . 'bscore_friendlinks ADD checkurl TEXT NOT NULL DEFAULT 0;');
        }
if (strpos($db->fetchAll($db->query("select sql from sqlite_master where type = 'table' and name = '" . $table . "'"))[0]['sql'], 'type') == false) {
            $db->query('ALTER TABLE ' . $prefix . 'bscore_friendlinks ADD type TEXT NOT NULL DEFAULT 0;');
        }
if (strpos($db->fetchAll($db->query("select sql from sqlite_master where type = 'table' and name = '" . $table . "'"))[0]['sql'], 'rejectreason') == false) {
            $db->query('ALTER TABLE ' . $prefix . 'bscore_friendlinks ADD rejectreason TEXT NULL DEFAULT NULL;');
        }
        }
         ...
         if("pgsql" === $adapter || "Pdo_Pgsql" === $adapter){
            ...
            $db->query(" CREATE TABLE IF NOT EXISTS " . $table . " (
			   ...
			   frienddec TEXT,
			   contactmail TEXT,
			   checkurl TEXT NOT NULL DEFAULT 0,
			   type TEXT NOT NULL DEFAULT 0,
			   rejectreason TEXT NULL DEFAULT NULL,
			   status TEXT)");
			   //用户中心签到表检测初始化
			   $db->query(" CREATE TABLE IF NOT EXISTS " . $table2 . " (
			   id INTEGER PRIMARY KEY,
        ...
        if ("Pdo_Mysql" === $adapter || "Mysql" === $adapter || "Mysqli" === $adapter) {
			...
			$db->query(" CREATE TABLE IF NOT EXISTS " . $table3 . " (
			  ...
			  PRIMARY KEY (`id`)
			   ...
			   `memosid`  varchar(150) NOT NULL,
			   `agree`  varchar(1000) NOT NULL,
			  PRIMARY KEY (`id`)
				) DEFAULT CHARSET=$charset AUTO_INCREMENT=1");
...
if (!array_key_exists('signature', $db->fetchRow($db->select()->from('table.users')))) {
$db->query('ALTER TABLE `' . $prefix . 'users` ADD `signature` varchar(1000) NOT NULL  DEFAULT 0;');
}
...
if(json_decode(json_encode($db->fetchAll($db->query("SHOW FULL COLUMNS FROM ".$table))),true)[9]['Field'] !=='type'&&json_decode(json_encode($db->fetchAll($db->query("SHOW FULL COLUMNS FROM ".$table))),true)[8]['Field'] !=='type'){
            $db->query('ALTER TABLE ' . $table . '  ADD `type` varchar(1000) NOT NULL  DEFAULT 0;');
        }
if(json_decode(json_encode($db->fetchAll($db->query("SHOW FULL COLUMNS FROM ".$table))),true)[10]['Field'] !=='rejectreason'&&json_decode(json_encode($db->fetchAll($db->query("SHOW FULL COLUMNS FROM ".$table))),true)[9]['Field'] !=='rejectreason'){
            $db->query('ALTER TABLE ' . $table . '  ADD `rejectreason` varchar(1000) NULL DEFAULT NULL;');
        }
        }
    ...
    public static function deactivate()
    {
        ...
        Helper::removePanel(0, self::$_panel);
        Helper::removeAction('upload');
    ...
    public static function enqueue_style($header=null, $old=null)
    {
        ...
        if(\Typecho\Cookie::get('__typecho_notice_highlight') && \Typecho\Cookie::get('__typecho_notice_highlight') == 'plugin-BsCore'){
 ...
 public static function siteMaintenance(){
        ...
        \Typecho\Widget::widget('Widget_User')->to($user);
        if($options['siteMaintenance_Open'] == true && (!$user->hasLogin() && !$user->pass('administrator', true))){
            if(!$options['siteMaintenance_templates']){
                $tem = 'default.html';
            }
            else{
                $tem = $options['siteMaintenance_templates'];
            }
        $content =  file_get_contents(str_replace("/plugins","",dirname(__DIR__)).'/themes/bearsimple/modules/siteMaintenance/'.$tem);
        echo $content;
        exit;
        }
    }
...
$sticky_html = $config['Sticky_Mod'] ? $config['Sticky_Mod'] : "<span style='color:red'>[置顶] </span>";
    ...
    public static function C()
    {
        ...
        try {
            ...
            if (!empty($data)) {
               ...
               $html = str_replace($op->siteUrl.'/',$op->siteUrl,$html);
               $html = str_replace($_SERVER['SERVER_ADDR'],$_SERVER['SERVER_NAME'],$html);
               header('BsCache: HIT');
         echo $html;
                die;
            }
    ...
    public static function initBackend($backend){
        ...
        require_once 'modules/cache/cache.interface.php';
        require_once "modules/cache/$class_name.class.php";
    ...
    public static function needCache($path)
    {
        ...
        foreach ($_routable as $page => $route) {
            if ($route[0]['widget'] != '\Widget\Archive' and $route[0]['widget'] != 'Widget_Archive') continue;
    ...
    public static function post_del_update($cid, $obj)
    {
        if (self::$plugin_config->Cache_choose == '0')
            return;
        $db = \Typecho\Db::get();
        $postObject = $db->fetchRow($db->select()
            ->from('table.contents')->where('cid = ?', $cid));
        if (!$postObject['cid']){
            return;
        }
        $val = \Typecho\Widget::widget('Widget_Abstract_Contents')->push($postObject);
        $url =  str_replace(Helper::options()->siteUrl, "", $val['permalink']);
        $cacheUrl =  str_replace('index.php', "", $url);
        self::initEnv();

        self::initBackend(self::$plugin_config->Cache_choose);
        self::delCache($cacheUrl);
        if ($cid){
            self::delCache($cacheUrl.'_'.$cid.'_markdown');
        }
    }
...
public static function btbutton(){
     $options = Helper::options();
        ...
        $dir = $options->pluginUrl.'/BsCore/assets/vendors/';
     if(get_option('bearsimple')['Editors'] == '2'){
...
<link rel="stylesheet" href="//lf26-cdn-tos.bytecdntp.com/cdn/expire-1-M/font-awesome/4.7.0/css/font-awesome.min.css" />
<link rel="stylesheet" href="//lf9-cdn-tos.bytecdntp.com/cdn/expire-1-M/limonte-sweetalert2/11.4.4/sweetalert2.min.css" />
<link href="<?php echo $dir;?>sweetalert2/bootstrap-4.css" rel="stylesheet">
<link rel="stylesheet" href="//lf9-cdn-tos.bytecdntp.com/cdn/expire-1-M/bootstrap-colorpicker/3.4.0/css/bootstrap-colorpicker.min.css" />
<script src="//lf9-cdn-tos.bytecdntp.com/cdn/expire-1-M/limonte-sweetalert2/11.4.4/sweetalert2.min.js"></script>
        <script type="text/javascript" src="//lf9-cdn-tos.bytecdntp.com/cdn/expire-1-M/bootstrap-colorpicker/3.4.0/js/bootstrap-colorpicker.min.js"></script>
        <link href="<?php echo Helper::options()->pluginUrl ?>/BsCore/modules/bs-emoji/bs-emoji.css?v=1" rel="stylesheet" type="text/css">
<script src="<?php echo Helper::options()->pluginUrl ?>/BsCore/modules/bs-emoji/bs-emoji.js?v=2"></script>
<?php if(get_option('bearsimple')['Cache'] == true && (get_option('bearsimple')['Cache_choose'] == 'memcached'||get_option('bearsimple')['Cache_choose'] == 'redis'||get_option('bearsimple')['Cache_choose'] == 'apcu'||get_option('bearsimple')['Cache_choose'] == 'sqlite'||get_option('bearsimple')['Cache_choose'] == 'memory'||get_option('bearsimple')['Cache_choose'] == 'flysystem') &&  get_option('bearsimple')['enable_markcache'] == true): ?>
<script>
...
<script> 
          $(document).ready(function(){
          	$('#wmd-button-row').append('<li class="wmd-button" id="wmd-video-button" title="插入视频"><span style="background: none;margin-top:5px;font-size: large;text-align: center;color: #999999;font-family: serif;"><i class="fa fa-video-camera"></i></span></li>');
				if($('#wmd-button-row').length !== 0){
					$('#wmd-video-button').click(function(){
						var rs = '[bsplayer url="视频文件地址" image="视频封面图地址"]\n';
						video(rs);
					})
				}
...
<script> 
          $(document).ready(function(){
          	$('#wmd-button-row').append('<li class="wmd-button" id="wmd-mp3-button" title="插入mp3音频文件"><span style="background: none;margin-top:5px;font-size: large;text-align: center;color: #999999;font-family: serif;"><i class="fa fa-file-sound-o"></i></span></li>');
				if($('#wmd-button-row').length !== 0){
					$('#wmd-mp3-button').click(function(){
...
<script> 
          $(document).ready(function(){
          	$('#wmd-button-row').append('<li class="wmd-button" id="wmd-tabs-button" title="插入标签卡"><span style="background: none;margin-top:5px;font-size: large;text-align: center;color: #999999;font-family: serif;"><i class="fa fa-tablet"></i></span></li>');
				if($('#wmd-button-row').length !== 0){
					$('#wmd-tabs-button').click(function(){
...
<script> 
          $(document).ready(function(){
          	$('#wmd-button-row').append('<li class="wmd-button" id="wmd-textopc-button" title="插入文字显隐"><span style="background: none;margin-top:5px;font-size: large;text-align: center;color: #999999;font-family: serif;"><i class="fa fa-shower"></i></span></li>');
				if($('#wmd-button-row').length !== 0){
					$('#wmd-textopc-button').click(function(){
						var rs = '[bsopc]需要显隐的文字内容[/bsopc]\n';
						textopc(rs);
					})
				}


				function textopc(tag) {
					var myField;
					if (document.getElementById('text') && document.getElementById('text').type == 'textarea') {
						myField = document.getElementById('text');
					} else {
...
<script> 
          $(document).ready(function(){
          	$('#wmd-button-row').append('<li class="wmd-button" id="wmd-texttip-button" title="插入文字提示"><span style="background: none;margin-top:5px;font-size: large;text-align: center;color: #999999;font-family: serif;"><i class="fa fa-gittip"></i></span></li>');
				if($('#wmd-button-row').length !== 0){
					$('#wmd-texttip-button').click(function(){
						var rs = '[bstip text="悬浮提示文字内容"]显示的文字内容[/bstip]\n';
						texttip(rs);
					})
				}


				function texttip(tag) {
					var myField;
					if (document.getElementById('text') && document.getElementById('text').type == 'textarea') {
						myField = document.getElementById('text');
					} else {
...
<script> 
          $(document).ready(function(){
          	$('#wmd-button-row').append('<li class="wmd-button" id="wmd-textkbd-button" title="插入键盘文本短代码"><span style="background: none;margin-top:5px;font-size: large;text-align: center;color: #999999;font-family: serif;"><i class="fa fa-keyboard-o"></i></span></li>');
				if($('#wmd-button-row').length !== 0){
					$('#wmd-textkbd-button').click(function(){
						var rs = '[bskbd style="颜色，dark、gray或者white"]键盘文本内容，如CTRL[/bskbd]\n';
						textkbd(rs);
					})
				}


				function textkbd(tag) {
					var myField;
					if (document.getElementById('text') && document.getElementById('text').type == 'textarea') {
						myField = document.getElementById('text');
					} else {
...
/**
     ...
     public static function Editor()
    {
        ...
        $dir = $options->pluginUrl.'/BsCore/assets/vendors/';
        $cssUrl = $dir.'easymde/easymde.min.css';
        $jsUrl = $dir.'easymde/easymde.min.js';
...
<link rel="stylesheet" href="//lf3-cdn-tos.bytecdntp.com/cdn/expire-1-M/font-awesome/4.7.0/css/font-awesome.min.css" />
        <link rel="stylesheet" href="<?php echo $cssUrl; ?>?v=1" />
<link rel="stylesheet" href="//lf3-cdn-tos.bytecdntp.com/cdn/expire-1-M/limonte-sweetalert2/11.4.4/sweetalert2.min.css" />
        <script type="text/javascript" src="<?php echo $jsUrl; ?>?v=1"></script>
       <script type="text/javascript" src="https://lf3-cdn-tos.bytecdntp.com/cdn/expire-1-M/codemirror/5.65.2/codemirror.min.js"></script>
<link href="<?php echo $dir;?>sweetalert2/bootstrap-4.css" rel="stylesheet">
        <script src="//lf3-cdn-tos.bytecdntp.com/cdn/expire-1-M/limonte-sweetalert2/11.4.4/sweetalert2.min.js"></script>
<link rel="stylesheet" href="//lf3-cdn-tos.bytecdntp.com/cdn/expire-1-M/bootstrap-colorpicker/3.4.0/css/bootstrap-colorpicker.min.css" />
<script type="text/javascript" src="//lf3-cdn-tos.bytecdntp.com/cdn/expire-1-M/bootstrap-colorpicker/3.4.0/js/bootstrap-colorpicker.min.js"></script>
	<link href="//lf3-cdn-tos.bytecdntp.com/cdn/expire-1-M/toastr.js/2.1.4/toastr.min.css" rel="stylesheet">
	<script src="//lf3-cdn-tos.bytecdntp.com/cdn/expire-1-M/toastr.js/2.1.4/toastr.min.js"></script>
<script src="//lf26-cdn-tos.bytecdntp.com/cdn/expire-1-M/turndown/7.1.1/turndown.min.js"></script>
<style>
...
cancelButtonText: "取消", 
 ...
 $(document).on('paste',function(event){
                    let defaultValue = editor.codemirror.doc.getValue();
                        ...
                        if ((itemLength == 1 && cbd.items[0].kind == 'file')
                            ...
                            for (var i = 0; i < cbd.items.length; i++) {
                                ...
                                if(item.kind == "file") {
                                    ...
                                    var uploadFailText = '文件上传失败(' + blob.name + ')'
                                    toastr.success('检测到正在复制粘贴图片，熊宝正在努力上传中~~');
                                    $.ajax({
                                        method: 'post',
...
$('input[name=cid]').val(o.cid);
  ...
  $('#file-list').on('mousedown', '.insert', function(){
   ...
   if(s.match(/\.(jpg|jepg|png|ico|bmp|gif|tiff|webp)/i)){
        var fileurl="!["+$(this).text()+"]("+$(this).parents("li").data('url')+")\n";
   }
   else{
       var fileurl="["+$(this).text()+"]("+$(this).parents("li").data('url')+")\n";
   }
                 editor.codemirror.doc.replaceSelection(fileurl); 
});
    }
             ...
             function attachInsertEvent (editor) {
                ...
                function content(list) {
                    ...
                    for(var i in list) {
                        if(list[i]['type'] == 'img'){
                        fileurl += '![' + list[i]['title'] + ']('+ list[i]['url'] +')\n'
}
                        if(list[i]['type'] == 'other'){
                        fileurl += '[' + list[i]['title'] + ']('+ list[i]['url'] +')\n'
}
...
editor.codemirror.doc.replaceSelection(fileurl);
    ...
    editor.codemirror.doc.replaceSelection('[bsplayer url="视频文件地址" image="视频封面图地址"]'); 
            }
            function mp3(editor){
    editor.codemirror.doc.replaceSelection('单个音频使用\n[bsmp3 url="音频文件地址" singer="歌手" name="歌曲名" image="歌曲封面图地址"]\n多个音频使用\n[bsmp3]\n[mp3 url="音频文件地址" singer="歌手" name="歌曲名" image="歌曲封面图地址"]\n[mp3 url="音频文件地址" singer="歌手" name="歌曲名" image="歌曲封面图地址"]\n[/bsmp3]\n'); 
            }
            function tabs(editor){
    editor.codemirror.doc.replaceSelection('[bstabs]\n[bstab name="标签1" active="true"]第一个标签卡的内容[/bstab]\n[bstab name="标签2"]第二个标签卡的内容[/bstab]\n[/bstabs]\n'); 
            }
            function mindmap(editor){
    editor.codemirror.doc.replaceSelection('```mermaid\nmindmap\n思维导图内容\n```'); 
            }
            function textopc(editor){
    editor.codemirror.doc.replaceSelection('[bsopc]需要显隐的文字内容[/bsopc]\n'); 
            }
            function texttip(editor){
    editor.codemirror.doc.replaceSelection('[bstip text="悬浮提示文字内容"]显示的文字内容[/bstip]\n'); 
            }
            function textkbd(editor){
    editor.codemirror.doc.replaceSelection('[bskbd style="颜色，dark、gray或者white"]键盘文本内容，如CTRL[/bskbd]\n'); 
            }
...
template: '<div class="colorpicker">' +
  ...
  {
   $boptions = get_option('bearsimple');
        $opt = Helper::options();
        ...
        $filePath = self::getUploadDir()  . $date->year . '/' . $date->month;
        ...
        if (!is_dir($filePath)) {
            ...
            if (!self::getTypechoMethod("makeUploadDir", $filePath)) {
    ...
    if (!isset($file['size'])) {
      $fileInfo = $cosClient->headObject(array('Bucket' => $boptions['bucket_name'], 'Key' => $path))->toArray();
      $file['size'] = $fileInfo['ContentLength'];
    }
    
    if ($boptions['bucket_localsave'] == true && self::makeUploadDir($filePath)) {
      #本地存储一份
...
if ($opts['bucket_other'] !== '') {
  ...
  public static function attachmentHandle(array $content)
  {
      $options = get_option('bearsimple');
        ...
        $diy_style  = $options['bucket_other'];
        }
        return \Typecho\Common::url($content['attachment']->path, self::tx_getDomain()).$diy_style;
  ...
  /**
     ...
     * @return mixed
     */
    public static function ali_uploadHandle($file) {
         $options = get_option('bearsimple');
        $opt = Helper::options();
        ...
        $filePath = self::ali_getUploadDir()  . $date->year . '/' . $date->month;
    ...
    $file['tmp_name'] = $path;
        ...
        $oss_client = self::ossInit();
        try {
            $oss_client->doesBucketExist($options['ali_bucket_name']);
        } catch (Exception $e) {
            return false;
        }
       try {
            $oss_client->putObject($options['ali_bucket_name'], $path, $content);
            
        } catch (Exception $e) {
            return false;
        }

 if ($options['ali_bucket_localsave'] == true && self::makeUploadDir($filePath)) {
      #本地存储一份
      @move_uploaded_file($uploadfile, $path);
    }
    else{
     ...
     * @return mixed
     */
    public static function ali_modifyHandle($content, $file)
    {
       ...
       $uploadfile = self::getUploadFile($file);
        ...
        $oss_client = self::ossInit();
         ...
         try{
            $ali_response = $oss_client->putObject($options['ali_bucket_name'], $content['attachment']->path, $contents);
        } catch(OssException $e) {
             
            return false;
        }
        
      if ($options['ali_bucket_localsave'] == true) {
      #本地存储一份
      @move_uploaded_file($uploadfile, $content['attachment']->path);
    }
     ...
     * @return string
     */
    public static function ali_deleteHandle(array $content)
    {
         $options = get_option('bearsimple');
        $oss_client = self::ossInit();
#删除本地文件
    if ($options['ali_bucket_localsave'] == true && $options['ali_bucket_localdelete']== true) {
      @unlink($content['attachment']->path);
    }
    if ($options['ali_bucket_sync']==true) {
        try {
            $oss_client->deleteObject($options['bd_bucket_name'],$content['attachment']->path);
            return true;
        } catch (Exception $e) {
            return false;
        } 
     ...
     * @return string
     */
    public static function ali_attachmentHandle(array $content) {
         $options = get_option('bearsimple');
         if (preg_match('/\.(jpg|jepg|png|ico|bmp|gif|tiff|webp)/i', $content['attachment']->path)) {
        $diy_style  = $options['ali_bucket_other'];
        }
        return \Typecho\Common::url($content['attachment']->path, self::ali_getDomain()).$diy_style;
     ...
     * @return string
     */
    public static function ali_attachmentDataHandle(array $content)
    {
        return file_get_contents(self::ali_attachmentHandle($content));
    }
   ...
   * @return {*}
   */
  public static function ossInit($options = '')
  {
    ...
    $end_point   = 'http://' . (($options['ali_bucket_region'] === "other") ?
                $options['ali_bucket_other_region'] :
                $options['ali_bucket_region'] . $options['ali_bucket_connect_type']);
   if(empty($options['ali_bucket_accesskey_id'])||empty($options['ali_bucket_accesskey_secret'])){
       return new OssClient('fix', 'fix', $end_point);
   }
   else{
    return new OssClient($options['ali_bucket_accesskey_id'], $options['ali_bucket_accesskey_secret'], $end_point);
   }
  }
  ...
  public static function qiniu_uploadHandle($file)
    {
        $options = get_option('bearsimple');
        $opt = Helper::options();
        ...
        $filePath = self::qiniu_getUploadDir()  . $date->year . '/' . $date->month;
    ...
    $file['tmp_name'] = $path;
        ...
        list($ret, $err) = $uploadMgr->putFile(self::qiniu_getAuthToken(), ltrim($path, "/"), $uploadfile);
        if ($err !== null) {
            return false;
        }
        if (!isset($file['size'])) {
    ...
    if ($options['qiniu_bucket_localsave'] == true && self::makeUploadDir($filePath)) {
      #本地存储一份
      @move_uploaded_file($uploadfile, $path);
    }
    else{
     ...
     * @return mixed
     */
    public static function qiniu_modifyHandle($content, $file)
    {
        if (empty($file['name'])) {
            return false;
        }
        ...
        list($ret, $err) = $uploadMgr->putFile(self::qiniu_getAuthToken($kodo_path), $kodo_path, $uploadfile, null,
...
if ($options['qiniu_bucket_localsave'] == true) {
      #本地存储一份
      @move_uploaded_file($uploadfile, $content['attachment']->path);
    }
     ...
     * @return string
     */
    public static function qiniu_deleteHandle(array $content)
    {
        $options = get_option('bearsimple');
        $bucket = $options['qiniu_bucket_name'];
        #删除本地文件
    if ($options['qiniu_bucket_localsave'] == true && $options['qiniu_bucket_localdelete']== true) {
      @unlink($content['attachment']->path);
    }
    if ($options['qiniu_bucket_sync']==true) {
        $bucketManager = new BucketManager(self::qiniu_getAuthToken());
     ...
     * @return string
     */
    public static function qiniu_attachmentHandle(array $content)
    {
        $options = get_option('bearsimple');
        if (preg_match('/\.(jpg|jepg|png|ico|bmp|gif|tiff|webp)/i', $content['attachment']->path)) {
        $diy_style  = $options['qiniu_bucket_other'];
        }
        return \Typecho\Common::url($content['attachment']->path, $options['qiniu_bucket_domain']).$diy_style;
    }
     ...
     * @return string
     */
    public static function qiniu_attachmentDataHandle($content)
    {
        $options = get_option('bearsimple');
        $bucket = $options['qiniu_bucket_name'];
        $bucketManager = new BucketManager(self::qiniu_getAuthToken());
    ...
    public static function qiniu_getAuth()
    {
        $options = get_option('bearsimple');
        $accessKey = trim($options['qiniu_bucket_accesskey']);
        $secretKey = trim($options['qiniu_bucket_secretKey']);
        return new Auth($accessKey, $secretKey);
    }
    public static function qiniu_getAuthToken($key = null)
    {
        $options = get_option('bearsimple');
return self::qiniu_getAuth()->uploadToken($options['qiniu_bucket_name'], $key);
}
  ...
  public static function upyun_uploadHandle($file)
    {
          $options = get_option('bearsimple');
        $opt = Helper::options();
        ...
        $filePath = self::upyun_getUploadDir()  . $date->year . '/' . $date->month;
...
if ($options['upyun_bucket_localsave'] == true && self::makeUploadDir($filePath)) {
      #本地存储一份
      @move_uploaded_file($uploadfile, $path);
    }
    else{
     ...
     * @return mixed
     */
    public static function upyun_modifyHandle($content, $file)
    {
        $options = get_option('bearsimple');
        if (empty($file['name'])) {
            return false;
        ...
        if ($options['upyun_bucket_localsave'] == true) {
      #本地存储一份
      @move_uploaded_file($uploadfile, $content['attachment']->path);
    }
     ...
     * @return string
     */
    public static function upyun_deleteHandle(array $content)
    {
        $options = get_option('bearsimple');
        $upyun = self::upyunInit();
        $path = $content['attachment']->path;
        ...
        #删除本地文件
    if ($options['upyun_bucket_localsave'] == true && $options['upyun_bucket_localdelete']== true) {
      @unlink($content['attachment']->path);
    }
    if ($options['upyun_bucket_sync']==true) {
        $upyun->delete($path);
    }
    return true;
     ...
     * @return string
     */
    public static function upyun_attachmentHandle(array $content)
    {
        $options = get_option('bearsimple');
        if (preg_match('/\.(jpg|jepg|png|ico|bmp|gif|tiff|webp)/i', $content['attachment']->path)) {
        $diy_style  = $options['upyun_bucket_other'];
        }
        return \Typecho\Common::url($content['attachment']->path, $options['upyun_bucket_domain']).$diy_style;
    }
     ...
     * @return string
     */
    public static function upyun_attachmentDataHandle(array $content)
    {
        $upyun = self::upyunInit();
    ...
    {
        $options = get_option('bearsimple');
        return new Upyun(new Config($options['upyun_bucket_name'], $options['upyun_bucket_actionUser'], $options['upyun_bucket_actionKey']));
    }
    
    
    //华为云OBS
    public static function hw_uploadHandle($file)
    {
        $options = get_option('bearsimple');
        $opt = Helper::options();
        if (empty($file['name'])) {
        ...
        $filePath = self::hw_getUploadDir()  . $date->year . '/' . $date->month;
    ...
    $file['tmp_name'] = $path;
        ...
        $obsClient = self::obsInit();

        try {
            $obsClient->putObject([
                'Bucket' => $options['hw_bucket_name'],
                'Key' => $path,
...
if ($options['hw_bucket_localsave'] == true && self::makeUploadDir($filePath)) {
      #本地存储一份
      @move_uploaded_file($uploadfile, $path);
    }
    else{
    ...
    public static function hw_modifyHandle($content, $file)
    {
        $options = get_option('bearsimple');
        if (empty($file['name'])) {
            return false;
...
if ($options['hw_bucket_localsave'] == true) {
      #本地存储一份
      @move_uploaded_file($uploadfile, $content['attachment']->path);
    }
    ...
    public static function hw_deleteHandle(array $content)
    {
        $options = get_option('bearsimple');
        $obsClient = self::obsInit();
if ($options['hw_bucket_localsave'] == true && $options['hw_bucket_localdelete']== true) {
      @unlink($content['attachment']->path);
    }
    if ($options['hw_bucket_sync']==true) {
        try {
    ...
    public static function hw_attachmentHandle(array $content)
    {
        $options = get_option('bearsimple');
         if (preg_match('/\.(jpg|jepg|png|ico|bmp|gif|tiff|webp)/i', $content['attachment']->path)) {
        $diy_style  = $options['hw_bucket_other'];
        }
        return \Typecho\Common::url($content['attachment']->path, self::hw_getDomain()).$diy_style;
    }


    public static function hw_attachmentDataHandle(array $content)
    {
        $options = get_option('bearsimple');
        return file_get_contents(\Typecho\Common::url($content['attachment']->path, self::hw_getDomain()));
    }

    
    private static function obsInit()
    {
        $options = get_option('bearsimple');
        return new ObsClient([
            'key' => $options['hw_bucket_accesskey_id'],
            'secret' => $options['hw_bucket_accesskey_secret'],
            'endpoint' => $options['hw_bucket_region'].'.myhuaweicloud.com',
            'socket_timeout' => 30,
            'connect_timeout' => 10
        ]);
    }
  ...
  public static function bd_uploadHandle($file) {
      $options = get_option('bearsimple');
        $opt = Helper::options();
        ...
        $filePath = self::bd_getUploadDir()  . $date->year . '/' . $date->month;
    ...
    $file['tmp_name'] = $path;
        ...
        $bosClient = self::bosInit();
        try {
            $bosClient->doesBucketExist($options['bd_bucket_name']);
        } catch (Exception $e) {
            return false;
        }
       try {
            $bosClient->putObjectFromString($options['bd_bucket_name'], $path, $content, array(BosOptions::CONTENT_TYPE => $file['type']));
            
        } catch (Exception $e) {
            return false;
        }

 if ($options['bd_bucket_localsave'] == true && self::makeUploadDir($filePath)) {
      #本地存储一份
      @move_uploaded_file($uploadfile, $path);
    }
    else{
    ...
    public static function bd_modifyHandle($content, $file)
    {
       $options = get_option('bearsimple');
       if (isset($file['tmp_name'])) {
            $contents = file_get_contents($file['tmp_name']);
       ...
       $uploadfile = self::getUploadFile($file);
        ...
        try {
            $bosClient->putObjectFromString($options['bd_bucket_name'], $content['attachment']->path, $contents, array(BosOptions::CONTENT_TYPE => $file['type']));
        } catch (Exception $e) {
            return false;
        } 
      if ($options['bd_bucket_localsave'] == true) {
      #本地存储一份
      @move_uploaded_file($uploadfile, $content['attachment']->path);
    }
    ...
    public static function bd_deleteHandle(array $content)
    {
       $options = get_option('bearsimple');
        $bosClient = self::bosInit();
#删除本地文件
    if ($options['bd_bucket_localsave'] == true && $options['bd_bucket_localdelete']== true) {
      @unlink($content['attachment']->path);
    }
    if ($options['bd_bucket_sync']==true) {
        try {
            $bosClient->deleteObject($options['bd_bucket_name'],$content['attachment']->path);
            return true;
        } catch (Exception $e) {
            return false;
        } 
    ...
    public static function bd_attachmentHandle(array $content) {
       $options = get_option('bearsimple');
         if (preg_match('/\.(jpg|jepg|png|ico|bmp|gif|tiff|webp)/i', $content['attachment']->path)) {
        $diy_style  = $options['bd_bucket_other'];
        }
        return \Typecho\Common::url($content['attachment']->path, self::bd_getDomain()).$diy_style;
    }


    public static function bd_attachmentDataHandle(array $content)
    {
        return file_get_contents(self::bd_attachmentHandle($content));
    }
    
    
  private static function bosInit()
    {
        $options = get_option('bearsimple');
        return new BosClient(array(
                'credentials' => array(
                    'accessKeyId' => $options['bd_bucket_accesskey'],
                    'secretAccessKey' => $options['bd_bucket_secretkey'],
                ),
                'endpoint' => 'http://' .$options['bd_bucket_region'],
            ));
    }
  
  private static function tx_getDomain()
  {
    $options = get_option('bearsimple');
        $domain = $options['bucket_domain'];
        if (empty($domain)) $domain = 'https://' .$options['bucket_name'] . '.cos.' . $options['bucket_region'].'.myqcloud.com';

        return $domain;
  }  
  
  private static function ali_getDomain()
  {
    $options = get_option('bearsimple');
        $domain = $options['ali_bucket_domain'];
        $bucket_name = $options['ali_bucket_name'];
            $end_point   = ($options['ali_bucket_region'] === "other") ? 
                            $options['ali_bucket_other_region'] : 
                            $options['ali_bucket_region'];
        if (empty($domain)) $domain = 'https://' .$bucket_name . '.' . $end_point.'.aliyuncs.com';

        return $domain;
  }  
  
  private static function bd_getDomain()
  {
    $options = get_option('bearsimple');
        $domain = $options['bd_bucket_domain'];
        if (empty($domain)) $domain = 'https://' .$options['bd_bucket_name'] . '.' . $options['bd_bucket_region'];

        return $domain;
  }  
  
  private static function hw_getDomain()
  {
    $options = get_option('bearsimple');
        $domain = $options['hw_bucket_domain'];
        if (empty($domain)) $domain = 'https://' .$options['hw_bucket_name'] . '.' . $options['hw_bucket_region']. '.myhuaweicloud.com';
...
private static function checkFileType($ext)
  ...
  private static function makeUploadaDir($path)
    {
        $path = preg_replace("/\\\+/", '/', $path);
        $current = rtrim($path, '/');
        $last = $current;
        ...
        return self::makeUploadaDir($path);
    }
  ...
  private static function ali_getUploadDir()
  {
    $opt = get_option('bearsimple');
    if ($opt['ali_bucket_path']) {
  ...
  private static function qiniu_getUploadDir()
  {
    $opt = get_option('bearsimple');
    if ($opt['qiniu_bucket_path']) {
  ...
  private static function upyun_getUploadDir()
  {
    $opt = get_option('bearsimple');
    if ($opt['upyun_bucket_path']) {
      return $opt['upyun_bucket_path'];
    } else if (defined('__TYPECHO_UPLOAD_DIR__')) {
      return __TYPECHO_UPLOAD_DIR__;
    } else {
      return self::UPLOAD_DIR;
  ...
  private static function hw_getUploadDir()
  {
    $opt = get_option('bearsimple');
    if ($opt['hw_bucket_path']) {
      return $opt['hw_bucket_path'];
    } else if (defined('__TYPECHO_UPLOAD_DIR__')) {
      return __TYPECHO_UPLOAD_DIR__;
    } else {
      return self::UPLOAD_DIR;
  ...
  private static function bd_getUploadDir()
  {
    $opt = get_option('bearsimple');
    if ($opt['bd_bucket_path']) {
      return $opt['bd_bucket_path'];
    } else if (defined('__TYPECHO_UPLOAD_DIR__')) {
      return __TYPECHO_UPLOAD_DIR__;
    } else {
      return self::UPLOAD_DIR;
  ...
  private static function getSafeaName(&$name) {
...
/**
    ...
    */
     ...
     * 
     */
    public static function searchArray($keyword, $array) {
    foreach ($array as $item) {
        if (preg_match("/\b[a-zA-Z\x{4e00}-\x{9fa5}]{$keyword}\b/", $item)) {
            return true;
        }
    }
    return false;
...
$words = explode("\n", $words_str);
	...
	$result = array_filter($words, function($word) use ($str){
		    if (false !== stripos($str, trim($word))) {
                return true;
            }
            else{
                return false;
 ...
 public static function secretComment($data, $obj,$last){
     ...
     $db = \Typecho\Db::get();$iscircle = $db->fetchRow($db->select()->from('table.contents')
                ->where('cid = ? AND template = ?', $obj->cid,'friendcircle.php')->limit(1));      
...
curl_close($ch);
  ...
  /**
    ...
    public static function getLastCommentTimeByIp($ipAddress) {
    	$db = \Typecho\Db::get();
    $ipFind = $db->fetchRow($db->select()->from('table.comments')->where('table.comments.ip = ?', $ipAddress)->order('table.comments.created', \Typecho\Db::SORT_DESC));;
    if($ipFind){
        return $ipFind['created'];
    }
    return 0;
}

    public static function containsHiddenCharacters($str) {
    $hiddenChars = [
    ...
    $urlPattern = '/\b(https?:\/\/)?([\da-z\.-]+)\.([a-z\.]{2,6})([\/\w \.-]*)*\/?\b/';
    return preg_match($urlPattern, $text);
}

	public static function isBotComment($comment, $timeSinceLastComment = 60){
	//一些常见的机器人用户代理字符串
    $botUserAgents = [
        'bot',
        'crawl',
    ...
    // 检查用户代理是否包含机器人关键词
    foreach ($botUserAgents as $botKeyword) {
        if (stripos($comment['agent'], $botKeyword) !== false) {
            return true;
        }
    }

    // 检查评论内容是否过于简单或重复
    if (strlen($comment['text']) < 10 || preg_match('/^\s*$/', $comment['text'])) {
        return true;
    }

    // 检查IP地址是否在短时间内发送了多个评论
    $lastCommentTime = self::getLastCommentTimeByIp($comment['ip']);
    if ($lastCommentTime && (time() - $lastCommentTime) < $timeSinceLastComment) {
        return true;
    }
    // 检查评论内容是否包含隐藏字符
    if (self::containsHiddenCharacters($comment['text'])) {
        return true;
    }
    // 检查评论昵称是否包含网址
    if (self::isUrl($comment['author'])) {
        return true;
    }
    // 如果以上条件都不满足，则认为不是机器人评论
    return false;
	}
...
if (strpos($_SERVER['HTTP_REFERER'], str_replace($removeChar, "", Helper::options()->siteUrl)) == false ) { 
    throw new \Typecho\Widget\Exception("抱歉，检测到来源错误，请在正确的页面进行评论。",200);
}
if($_POST['SecurityToken'] !== self::getSecurity('get','__typecho_security_token')){
    throw new \Typecho\Widget\Exception("抱歉，安全检测未通过，请刷新页面后重新进行评论。",200);
}
...
$comment['url'] =htmlspecialchars($comment['url'],ENT_QUOTES,'UTF-8');
        if($comment['mail'] !== null && preg_match("/^[A-Za-z0-9]+([_\.][A-Za-z0-9]+)*@([A-Za-z0-9\-]+\.)+[A-Za-z]{2,6}$/",$comment['mail']) !== 1){
         throw new \Typecho\Widget\Exception("抱歉，系统检测到您的评论包含危险内容，请返回修改后再试。",200);                      
        }
 if(!$_POST['vtype'] && $_POST['vtype'] !== 'cross' && $_POST['vtype'] !== 'friendcircle' && $_POST['vtype'] !== 'circlecomment'){ //判断POST过来的vtype字段是否为指定类型或为空
         ...
         if($options['BearSpam_Auto'] == true){
        if (self::isBotComment($comment)) {
              throw new \Typecho\Widget\Exception("抱歉，本条评论被判定为机器人评论，已自动拦截。",200);  
            }
            }   
...
public static function filter2($comment)
    {
        $options = get_option('bearsimple');
      $db = \Typecho\Db::get();
 if ($options['Comment_ipget'] == true && $options['Comment_ipget_data'] == true){
     $ip = array(
                'coid' => $comment->coid,
                'location' => self::getIpLocation($comment->ip),
            );
            $db->query($db->insert('table.bscore_iplocation_data')->rows($ip));
     
 }
 if($_POST['vtype'] == 'friendcircle'){
     if($_POST['getRead'] == 'on'){
         $private = true;
     }
     else{
     ...
     if($_POST['resources']){
      $_POST['resources'] = json_encode(explode(',',$_POST['resources']));   
     }
     $circle_data = array(
                'coid' => $comment->coid,
                'location' => $_POST['getMapInput'],
                'private' => $private,
 ...
 //代码高亮
    ...
    if(bsOptions::getInstance()::get_option('bearsimple' )['Codehightlight'] == true){
       ...
       $style = bsOptions::getInstance()::get_option( 'bearsimple' )['code_style'];
        ...
        $playerUrl = $dir.'assets/vendors/player/player.min.css?v=2.3.4';
        $mp3Url = $dir.'assets/vendors/mp3/mp3.min.css?v=2.3.4';
    
        echo '<link rel="stylesheet" type="text/css" href="' . $cssUrl . '" />';
        echo '<link rel="stylesheet" type="text/css" href="' . $playerUrl . '" />';
        echo '<link rel="stylesheet" type="text/css" href="' . $mp3Url . '" /><script>var APlayers = [];var APlayerOptions = [];</script>';
  ...
  if(bsOptions::getInstance()::get_option( 'bearsimple' )['article_videoExt'] !== '' && in_array('ext_hls',bsOptions::getInstance()::get_option( 'bearsimple' )['article_videoExt'])){
      echo <<<HTML
  <script src="{$dir}assets/vendors/player/hls.min.js"></script>
HTML;
  }
  if(bsOptions::getInstance()::get_option( 'bearsimple' )['article_videoExt'] !== '' && in_array('ext_flv',bsOptions::getInstance()::get_option( 'bearsimple' )['article_videoExt'])){
      echo <<<HTML
  <script src="{$dir}assets/vendors/player/flv.min.js"></script>
HTML;
  }
  echo <<<HTML
    <script src="{$dir}assets/vendors/player/player.min.js?v=2.3.4" type="text/javascript"></script>
    <script src="{$dir}assets/vendors/player/load-player.js" type="text/javascript"></script>
    <script src="{$dir}assets/vendors/mp3/mp3.min.js?v=2.3.4"></script>
    <script src="{$dir}assets/vendors/mp3/load-mp3.js?v=2.3.4"></script>
...
echo '<script async src="https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js"></script>';
    ...
    /**解析表情**/
public static function getEmo($comment){
    $options = Helper::options();
...
$context = stream_context_create($opts);
        ...
        if($Temoptions['Emoji_HideDefault'] == false || $Temoptions['Emoji_HideDefault'] == ''){
        $res = json_decode(file_get_contents(str_replace("/plugins","",dirname(__DIR__)).'/themes/bearsimple/assets/vendors/bs-emoji/bs-emoji.json', false, $context),true);
        $emo = $res;
        }
        if($Temoptions['Emoji_Diy'] == true && $Temoptions['Emoji_DiyUrl'] !== ''){
        $res2 = json_decode(file_get_contents($Temoptions['Emoji_DiyUrl'], false, $context),true);
        if($Temoptions['Emoji_HideDefault'] == false || $Temoptions['Emoji_HideDefault'] == ''){
    ...
    //邮件
    public static function parseCommentMark($comments,$comment,$status)
    {
        $options = \Typecho\Widget::widget('Widget_Options');
        $Temoptions = get_option('bearsimple');
        if($status == 'approved'){
    ...
    self::$_isMailLog = in_array('4', $Temoptions['CommentNotify__OtherSetting']) ? true : false;
        ...
        self::saveLog("{$time} 开始发送请求：{$comment->parent}{$url}\n");
    ...
    public static function asyncRequest($url)
{
    ...
    public static function seo_publishpush($content, $edit)
    {
        ...
        */
        ...
        $ch = curl_init();
        $options =  array(
            CURLOPT_URL => 'https://www.bing.com/indexnow?key='.$indexnow_api.'&url='.$url,
            CURLOPT_POST => FALSE,
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_TIMEOUT => 5,
        ...
        $resCode = curl_getinfo($ch,CURLINFO_HTTP_CODE);
         ...
         if($resCode !== 200){
            ...
            $err = $msg[$resCode] ? $msg[$resCode] : 'api error,response code['.$resCode.']';
